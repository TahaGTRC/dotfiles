...
int main(int argc, char *argv[])
{
	...
	XftFont *font = XftFontOpenName(display, screen, font_pattern[0]);
	XftFont *fallback_font = XftFontOpenName(display, screen, font_pattern[1]);
	...
	for (;;)
	{
		...
		if (event.type == Expose)
		{
			...
			for (int i = 0; i < num_of_lines; i++) {
				...
				XftDrawStringUtf8(draw, &color, fallback_font, padding, line_spacing * i + text_height * (i + 1) + padding,
								  (FcChar8 *)lines[i], strlen(lines[i]));
			}
		}
